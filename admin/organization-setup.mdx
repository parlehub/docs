...
<Steps>
  ...
  <Step title="Connect Model Providers">
    Navigate to **Organization Settings → Model access** and add the AI providers your firm will use (for example, OpenAI, Anthropic, or Azure AI Foundry). Provider credentials are stored at the org level and made available to all projects. See the [Connecting Providers](/models/connecting-providers) guide for step-by-step instructions.
  </Step>
  <Step title="Set Your Currencies (If You Do Not Work in US Dollars)">
    If a provider bills you in GBP, EUR or CAD, set its **Quote currency** when you connect it — on each Azure AI Foundry deployment, or on the Google Gemini card — before you enter any rates. Then choose the currency your reports read in under **Organization Settings → Budgets & alerts → Reporting currency**. Each project's caps get their own budget currency when you create it. Everything defaults to USD. See [Currencies and Exchange Rates](/cost/currencies).
  </Step>
  ...
  <Step title="Set Org-Wide Spending Caps">
    Navigate to **Organization Settings → Billing** and, on the **Monthly spend cap** card, enter a **Cap (USD / month)** — a monthly US-dollar ceiling on total model spend for the entire org — then select **Save cap**. This acts as an outer backstop above all individual project caps and personal budgets. See [Org-Wide and Per-User AI Spending Caps](/admin/org-caps) for details.
  </Step>
</Steps>
...
<CardGroup cols={2}>
  ...
  <Card title="Audit Log" icon="scroll" href="/security/audit-log">
    Understand what ParleHub records in its audit log — membership, settings, budget and provider changes among them. This is a reference page, not a section of Settings: there is no audit screen or export in the app today.
  </Card>
