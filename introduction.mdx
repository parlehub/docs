...
<CardGroup cols={2}>
  ...
  <Card
    ...
    href="/cost/budgets"
  >
    Every project carries your own project code — a matter number, cost centre or expense code — straight through to the export. Spend and token usage break down by project, person, provider, or model over any date range.
  </Card>
  <Card
    title="Control"
    icon="sliders"
    href="/cost/budgets"
  >
    Give each project a monthly cap, a lifetime cap, or both, in USD, GBP, EUR or CAD. Pre-flight enforcement blocks a send before it takes the project past either one, an 80% warning shows before a cap is hit, and anyone who is blocked can request a higher limit right from the notice — with a short note for the Org Admin who approves it.
  </Card>
  <Card
  ...
  <Card
    ...
    href="/security/overview"
  >
    Single sign-on via Microsoft Entra ID or Google Workspace, files that never leave your tenant, an append-only audit log, and role-based access at the org and project level.
  </Card>
</CardGroup>

## How it fits together

A ParleHub **project** maps to a client engagement. It has a name, a project code you assign (your matter number or expense code), spending caps — monthly, lifetime, or both — in the currency you choose, and a set of members drawn from your organization. Inside a project, the team runs AI-assisted conversations using whichever models you've connected — Anthropic, OpenAI, Google Gemini, or Azure AI Foundry on your own API keys. Every token consumed is recorded against the project's budget in a single canonical ledger that drives both the real-time enforcement and the export report.
