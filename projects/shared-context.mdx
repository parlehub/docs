...
## Standing instructions: `agent.md`

Thread history is context the team accumulates. `agent.md` is context you write down once, deliberately, and it is the single highest-leverage thing to set up on a new project.

`agent.md` holds standing instructions for the agent — the equivalent of a project brief. Its contents are injected at the start of **every conversation in the project**, so nobody has to re-explain the client, the house style, or the ground rules each time.

<Steps>
  <Step title="Open Project Details">
    Scroll to the **agent.md** section.
  </Step>
  <Step title="Write the project's standing instructions">
    Useful things to include:

    - What the project is, who the client or stakeholder is, and what "done" looks like.
    - Conventions to follow — tone, terminology, document structure, file naming.
    - Where things live in the project's files, and what to read before answering.
    - Anything the agent must never do, for example: do not share figures outside the approved deck.
  </Step>
  <Step title="Save">
    Each save creates a new version. The panel shows the current version number and when it was last updated.
  </Step>
</Steps>

<Note>
  Editing project details, storage, and `agent.md` requires **Project Admin** on this project, or Org Admin. Everyone else sees it read-only. Every edit is written to the [audit log](/security/audit-log).
</Note>

<Tip>
  `agent.md` is project-scoped. Documents that are true for your whole firm — an expenses policy, a methodology, a staff handbook — belong in [organization knowledge](/knowledge/organization-knowledge) instead, where they reach every conversation in the organization and are quoted with a citation.
</Tip>
